...
<div class="table-responsive mt-5">
    <table class="table table-striped caption-top table-borderless" id="example">
        <div class="d-flex justify-content-end mb-3">
            <a href="/Admin/pesanan/create" class="btn btn-primary"><i class="fa-solid fa-plus"></i> Tambah Data</a>
        </div>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="text-nowrap fw-bold {{ ($data->pivot->status)=='Selesai' ? 'text-success' : 'text-warning' }}">{{ $data->pivot->status }}</td>
                <td class="d-flex gap-2">
                    <a href="/Admin/pesanan/{{ $data->pivot->kode_pesanan }}/edit" class="btn btn-outline-warning"><i class="fa-solid fa-pen-to-square"></i></a>
                    <form action="/Admin/pesanan/{{ $data->pivot->id }}" method="POST">
                        @method('DELETE')
                        @csrf
